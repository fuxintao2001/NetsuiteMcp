...
export class NetSuiteMCPTools {
	...
	async fetchTools(): Promise<unknown[]> {
		...
		if (!tools || !Array.isArray(tools.tools)) {
			throw new Error(
				"Invalid tools/list response: missing result.tools array",
			);
		}
				...
				for (const table of tableNames) {
					try {
						await cacheService.delete(
							accountId,
							`ns_getSuiteQLMetadata_${table}`,
						);
						await cacheService.delete(
							accountId,
							`ns_getRecordTypeMetadata_${table}`,
						);
					} catch {
	...
	async fetchCustomRecordMappings(): Promise<void> {
		...
		try {
			...
			for (const record of records) {
				const scriptId = String(record.scriptid || record.scriptId || "")
					.toUpperCase()
					.trim();
	...
	async seedMetadataFromLocalRecords(accountId: string): Promise<void> {
		try {
			...
			for (const recordType of highFrequencyTypes) {
				const cacheKey = this.metadataCacheKey("ns_getRecordTypeMetadata", {
					recordType,
				});
	 ...
	 */
	private async jsonRpcCallRaw<T>(
		method: string,
		params?: Record<string, unknown>,
	): Promise<T> {
		...
		const makeRequest = async (token: string): Promise<T> => {
			...
			if (response.data?.error) {
				...
				const codeMsg =
					rpcErr.code !== undefined ? ` (code: ${rpcErr.code})` : "";
				const dataMsg = rpcErr.data
					? ` - Details: ${JSON.stringify(rpcErr.data)}`
					: "";
				throw new Error(
					`NetSuite JSON-RPC Error${codeMsg}: ${errMsg}${dataMsg}`,
				);
			}
	...
	private slimSuiteQLResponse(result: unknown): unknown {
		...
		// Wrapped shape: { content: [{ text: '...' }] }
			...
			if (firstContent?.text && typeof firstContent.text === "string") {
				try {
					const parsed = JSON.parse(firstContent.text) as Record<
						string,
						unknown
					>;
					if (
						parsed.method === "custom_suiteql" &&
	...
	private async loadCustomRecordMappingsCache(): Promise<void> {
		...
		const mappingsObj = await cacheService.get<Record<string, number>>(
			accountId,
	...
	public extractDataArray(result: unknown): Array<Record<string, unknown>> {
		...
		const records = (data.data || data.records || []) as Array<
