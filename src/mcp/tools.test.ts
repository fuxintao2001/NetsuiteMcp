	...
	describe("executeTool", () => {
		it("should return cached metadata for metadata tools", async () => {
			...
			expect(cacheService.get).toHaveBeenCalledWith(
				"test-acc",
				"ns_getRecordTypeMetadata_customer",
			);
		});
		...
		it("should invalidate cache for related tables on SuiteQL error (self-heal)", async () => {
			...
			expect(cacheDelSpy).toHaveBeenCalledWith(
				"test-acc",
				"ns_getSuiteQLMetadata_customer",
			);
			expect(cacheDelSpy).toHaveBeenCalledWith(
				"test-acc",
				"ns_getSuiteQLMetadata_salesorder",
			);
		});
		...
		it("should retry once on 401 unauthorized errors with a forced refresh token", async () => {
			...
			httpPostSpy.mockRejectedValueOnce(err401).mockResolvedValueOnce({
				data: {
					result: mockResult,
				},
			});

			const result = await client.executeTool("ns_runCustomSuiteQL", {
				sqlQuery: "SELECT id FROM customer",
			});
			expect(result).toEqual(mockResult);
			expect(mockOAuthManager.forceRefreshToken).toHaveBeenCalledWith(
				"token-111",
			);
		...
		it("should retry on transient HTTP 502 and 504 errors", async () => {
			...
			httpPostSpy.mockRejectedValueOnce(err502).mockResolvedValueOnce({
				data: {
					result: mockResult,
				},
			});
