...
export function validateSuiteQL(sqlQuery: string): SuiteQLValidationResult {
	if (!sqlQuery || typeof sqlQuery !== "string" || sqlQuery.trim().length === 0) {
		return {
			valid: false,
			reason: "SuiteQL query cannot be empty.",
		};
	}
	...
	if (trimmed.includes("--") || trimmed.includes("/*") || trimmed.includes("*/") || trimmed.includes("#")) {
		return {
			valid: false,
			reason: "SuiteQL query contains prohibited SQL comments (-- or /* */ or #).",
		};
	}
	...
	if (withoutTrailingSemicolon.includes(";")) {
		return {
			valid: false,
			reason: "SuiteQL query contains multiple statements separated by semicolons. Only single queries are allowed.",
		};
	}
	...
	if (!/^(SELECT|WITH)\b/i.test(withoutTrailingSemicolon)) {
		return {
			valid: false,
			reason: "SuiteQL queries must begin with a SELECT or WITH statement.",
		};
	}
	...
	if (match) {
		return {
			valid: false,
			reason: `SuiteQL query contains disallowed mutation or DDL keyword: '${match[1]}'. Only read-only queries are permitted.`,
		};
	}
