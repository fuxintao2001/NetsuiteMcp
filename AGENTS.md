...
> 3. **Automatic Self-Healing (Syntax/Schema Only)**: On query syntax/schema error or empty results, parse the structured diagnostic response, inspect metadata via `ns_getSuiteQLMetadata`, fix the query, and retry (up to 3 retries) before declaring failure. (Excludes permission errors).
